...
class Louvain(object):
    ...
    def getBestPartition(self, graph, param=1.):
        # Initialise the vertices and egdes
        ...
        # Recursive process with a optimization phase and a aggregation phase
        while True:
    ...
    def computeModularity(self, node2com, edge_weights, param):
        """Compute the Fortunato (2010) modularity, i.e. modularity for
        undirected and weighted graphs."""
        q = 0
        all_edge_weights = sum([weight for start in edge_weights.keys()
                                for end, weight in edge_weights[start].items()]) / 2
        ...
        for com_id, nodes in com2node.items():
            ...
            cluster_weight = sum([edge_weights[node_pair[0]][node_pair[1]]
                                  for node_pair in node_combinations])
            tot = self.getDegreeOfCluster(nodes, node2com, edge_weights)
            q += (cluster_weight / (2 * all_edge_weights)) - \
                param * ((tot / (2 * all_edge_weights)) ** 2)
        return q
    ...
    def _runSecondPhase(self, node2com, edge_weights):
        ...
        for edge in node_pairs:
            new_edge_weights[new_node2com[node2com[edge[0]]]
                             ][new_node2com[node2com[edge[1]]]] += edge_weights[edge[0]][edge[1]]
